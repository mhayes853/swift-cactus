...
/// Result builder for composing prompt representable values.
@resultBuilder
  ...
  public var promptContent: CactusPromptContent {
    get throws {
      ...
      for content in self.contents {
        prompt.join(with: try content.promptContent, separator: _CactusPromptContext.separator)
      }
      return prompt
    }
  }
...
public struct _OptionalContent<Content: CactusPromptRepresentable>: CactusPromptRepresentable {
  let content: Content?

  public var promptContent: CactusPromptContent {
    get throws {
      try self.content?.promptContent ?? CactusPromptContent()
    }
  }
}
