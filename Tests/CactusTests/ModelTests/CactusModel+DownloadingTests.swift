...
@Suite
struct `CactusModelDownloading tests` {
  @Test
  func `Builds Hugging Face URL`() {
    let request = CactusModel.PlatformDownloadRequest.lfm2_700m(version: .v1_9)
    expectNoDifference(
      request.defaultURL.absoluteString,
      "https://huggingface.co/Cactus-Compute/LFM2-700M/resolve/v1.9/weights/lfm2-700m-int4.zip"
    )
  }

  @Test
  func `Builds Pro Hugging Face URL`() {
    let request = CactusModel.PlatformDownloadRequest.lfm2_5Vl_1_6b(pro: .apple, version: .v1_9)
    expectNoDifference(
      request.defaultURL.absoluteString,
      "https://huggingface.co/Cactus-Compute/LFM2.5-VL-1.6B/resolve/v1.9/weights/lfm2.5-vl-1.6b-int4-apple.zip"
    )
  }

  @Test
  func `Builds Moonshine URL`() {
    let request = CactusModel.PlatformDownloadRequest.moonshineBase(pro: .apple, version: .v1_9)
    expectNoDifference(
      request.defaultURL.absoluteString,
      "https://huggingface.co/Cactus-Compute/moonshine-base/resolve/v1.9/weights/moonshine-base-int4-apple.zip"
    )
  }

  @Test
  func `Builds Silero VAD URL`() {
    let request = CactusModel.PlatformDownloadRequest.sileroVad(quantization: .int8, version: .v1_7)
    expectNoDifference(
      request.defaultURL.absoluteString,
  ...
  func `Task Not Finished By Default`() async throws {
    let task = CactusModel.downloadModelTask(
  ...
  func `Download Model Successfully`() async throws {
    let progress = Lock([Result<CactusModel.DownloadProgress, any Error>]())
    let observedProgress = Lock([CactusModel.DownloadProgress]())
  ...
  func `Cancel Download From Concurrency Task`() async throws {
    let task = Task {
      try await CactusModel.downloadModel(
        request: .whisperSmall(),
        to: self.temporaryURL()
      )
    }
